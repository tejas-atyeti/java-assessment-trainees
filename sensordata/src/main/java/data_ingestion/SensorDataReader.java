        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (fields.length == 5) { // Assuming CSV has 3 columns: Name, Age, Email
                    LocalDate timestamp = LocalDate.from(LocalDateTime.parse(fields[0],formatter));
                    SensorType type = SensorType.valueOf(fields[1].toUpperCase());
                    double value = Double.parseDouble(fields[2]);
                    String unit = fields[3];
                    int locationId = Integer.parseInt(fields[4]);
                    SensorData data = new SensorData(timestamp,type,value,unit,locationId);
                    sensorData.add(data);
                }
                else{
